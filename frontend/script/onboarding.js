...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function requestOtp() {
        ...
        try {
            ...
            console.warn('Backend server not reachable, continuing in offline/demo mode:', err);
        }
    }
    ...
    if (onboardForm) {
        onboardForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                console.warn('Backend API connection failed, proceeding with client fallback:', err);
                // Graceful fallback for offline preview
                sessionStorage.setItem('km_temp_mobile', mobile);
                sessionStorage.setItem('km_temp_lang', language);
                window.location.href = 'details.html';
            }
        });
    }
